...
@objc public class DataKit: NSObject {
	...
	public func staminaAtLevel(level:Int) -> Int {
		return STAMINA.objectAtIndex(level) as! Int
	}
	
	public func levelAtStamina(stamina:Int) -> Int {
		for index in 1...150 {
			let level = 151 - index
			let value = STAMINA.objectAtIndex(level) as! Int
			if (value == stamina) {
				return level
			}
		}
	...
	public func expAtLevel(level:Int) -> Int {
		return EXP.objectAtIndex(level) as! Int
	}
